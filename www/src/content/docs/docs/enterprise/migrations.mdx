...
The CLI reads `database_name` from this file when no database is specified.
...
Either specify the database name or ensure your `wrangler.toml` has a `[[d1_databases]]` section with `database_name`.
